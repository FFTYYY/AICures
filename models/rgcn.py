...
class RConv(nn.Module):
	def __init__(self , in_feats , out_feats , norm="both"):
		...
		self.e_emb = nn.Embedding(4000 , in_feats * out_feats)
	...
	def reset_parameters(self):
		...
		init.uniform_(self.e_emb.weight , 0 , 1e-5)
	...
	def my_message_func(self , edges):
		...
		h_o = tc.matmul(o_w , h).view(-1 , self._out_feats)

		e_feat = edges.src["e"] * edges.dst["e"]
		emask = (edges.edges()[0] != edges.edges()[1]).long().to(h.device) #一样的为0，不一样的为1
		e_feat = e_feat * emask #一样的位置设为0
		e_w = self.e_emb(e_feat).view(-1 , self._out_feats , self._in_feats)
		h_e = tc.matmul(e_w , h).view(-1 , self._out_feats)
	...
	def forward(self , graph , feat):
		...
		graph.srcdata["e"] = graph.ndata["element"]
